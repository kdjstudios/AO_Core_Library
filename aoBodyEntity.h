...
public:
  ...
  void      SetVelocity(const float& new_Vel){v = new_Vel;}

  float     Mass()const{return m;}

  //Vector3 Side()const{return m_Side;}

  double     Thrust()const{return t;}
  void      SetThrust(double new_thrust){t = new_thrust;}

  double     DragCoefficient()const{return c;}
  void      SetDragCoefficient(double new_dragCoeffiecient){c = new_dragCoeffiecient;}
